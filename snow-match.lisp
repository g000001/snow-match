...
;; This is a CL port of Alex Shinn's MATCH package.
;; Ported by CHIBA Masaomi.
...
(define-syntax match-next
  (syntax-rules (:=>)
    ...
    ((match-next v g s (pat (:=> failure) . body) . rest)
     (with ((failure (gensym "failure-")))
       (let ((failure (lambda () (match-next v g s . rest))))
         (declare (ignorable failure))
         ;; match-one analyzes the pattern for us
         (match-one v pat g s (match-drop-ids (progn . body)) (funcall failure) ()))))
...
(define-syntax match-quasiquote
  (syntax-rules (unquote unquote-splicing quasiquote)
    ...
    ((:_ v (quasiquote p) g s sk fk i . depth)
     (match-quasiquote v p g s sk fk i :false . depth))
                   ...
                   (cond
                     ((= n tail-len)
                      (let ((id (reverse id-ls)) ***)
                        (match-one ls (r ***) :false :false (sk *** i) fk i) ))
                     ((consp ls)
                      (let ((w (car ls)))
                        (match-one w p (car ls) (set-car! ls)
                                   (match-drop-ids
...
(define-syntax match-vector-step
  (syntax-rules ()
    ...
    ((:_ v ((pat index) . rest) sk fk i)
     (with ((w (gensym "W-")))
       (let ((w (aref v index)))
         (match-one w pat (aref v index) (vector-set! v index)
     ...
     (with ((loop (gensym "LOOP-"))
      ...
      (let loop ((j n) (id-ls '()) ***)
        (if (>= j len)
            (let ((id (reverse id-ls)) ***) (sk *** i))
            (let ((w (aref v j)))
              (match-one w p (aref v j) (vetor-set! v j)
...
(define-syntax match-extract-vars
  (syntax-rules (:_ :___ :? $ = quote quasiquote and or not get! set!)
    ...
    ((match-extract-vars (quasiquote x) k i v)
     (match-extract-quasiquote-vars x k i v (:true)))
...
(define-syntax match-extract-quasiquote-vars
  (syntax-rules (quasiquote unquote unquote-splicing)
    ((match-extract-quasiquote-vars (quasiquote x) k i v d)
     (match-extract-quasiquote-vars x k i v (:true . d)))
    ((match-extract-quasiquote-vars (unquote-splicing x) k i v d)
     (match-extract-quasiquote-vars (unquote x) k i v d))
    ((match-extract-quasiquote-vars (unquote x) k i v (:true))
     (match-extract-vars x k i v))
    ((match-extract-quasiquote-vars (unquote x) k i v (:true . d))
     (match-extract-quasiquote-vars x k i v d))
    ((match-extract-quasiquote-vars (x . y) k i v (:true . d))
...
(define-syntax match-lambda*
  (syntax-rules ()
    ((:_ clause ***)
     (with ((expr (gensym "EXPR-")))
       (lambda (&rest expr) (match expr clause ***))))))
  ...
  (syntax-rules ()
    ...
    ((:_ x success-k failure-k)
     (with ((x (gensym "X-"))
            ...
            (sk (gensym "SK-"))
            (fk (gensym "FK-"))
            (abracadabra (gensym "ABRACADABRA-")))
     (let-syntax
         ((sym?
           (syntax-rules ()
